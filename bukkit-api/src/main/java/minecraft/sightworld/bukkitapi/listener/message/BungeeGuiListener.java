...
public class BungeeGuiListener extends BaseMessageListener<Player, BungeeGuiDto> {

    private final MessagingService<Player> messagingService;

    @Override
    public void onMessage(Player player, String messageChannel, BungeeGuiDto gui) {
            ...
            public void draw() {
                for (BungeeGuiItem item : gui.getItems()) {
                    ...
                    GuiItem guiItem = new GuiItem(itemBuilder.build(), (player1, clickType) -> {
                        ...
                        messagingService.sendMessage(player, item, "gui-acceptor");
