...
public abstract class PagedGui implements IGui {
    ...
    private static ItemStack NEXT_PAGE_ITEM(int str) {
        ItemStack next = new ItemStack(Material.SPECTRAL_ARROW);
        ItemMeta itemMeta = next.getItemMeta();
        itemMeta.setDisplayName("§aСледующая страница");
        List<String> lore = new ArrayList<>();
        lore.add("§7Нажмите для перехода");
        lore.add("§7на " + str + " страницу");
        itemMeta.setLore(lore);
        next.setItemMeta(itemMeta);
        return next;
    }

    private static ItemStack BACK_PAGE_ITEM(int str) {
        ItemStack back = new ItemStack(Material.SPECTRAL_ARROW);
        ItemMeta itemMeta = back.getItemMeta();
        itemMeta.setDisplayName("§aПредыдущая страница");
        List<String> lore = new ArrayList<>();
        lore.add("§7Нажмите для перехода");
        lore.add("§7на " + str + " страницу");
        itemMeta.setLore(lore);
    ...
    public void setPage(int pageId) {
        ...
        currentGui = new Gui(player, size / 9, title + (currentPage > 0 ? " ▸ " + (currentPage + 1) : "")) {
            ...
            public void draw() {
                ...
                if (hasBackPage())
                    setItem(size - 6, BACK_PAGE_ITEM(currentPage), (player1, clickType) -> {
                ...
                if (hasNextPage())
                    setItem(size - 4, NEXT_PAGE_ITEM(currentPage + 2), (player1, clickType) -> {
                        setPage(currentPage + 1);
                        PagedGui.this.open();
                    });
    ...
    public void setMarkupPrefix() {
        setMarkup(11, 12, 13, 14, 15, 20, 21, 22, 23, 24,
                29, 30, 31, 32, 33);
    }
